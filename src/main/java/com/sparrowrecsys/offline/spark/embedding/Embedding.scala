...
object Embedding {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val rawSampleDataPath = "/webroot/sampledatanew/ratings.csv"
    val embLength = 10

    val samples = processItemSequence(spark, rawSampleDataPath)
    val model = trainItem2vec(spark, samples, embLength, "item2vecEmbNew.csv", saveToRedis = false, "i2vEmb")
//    graphEmb(samples, spark, embLength, "itemGraphEmbNew.csv", saveToRedis = true, "graphEmb")
    generateUserEmb(spark, rawSampleDataPath, model, embLength, "userEmbNew.csv", saveToRedis = true, "uEmb")
  }
}
